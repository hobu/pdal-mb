...
def pdal_pipeline(event, context):
    ...
    if record['eventSource'] == 'aws:sqs':
        ...
        queueAccount = queueARN.split(':')[-2]
        queueURL = sqs_client.get_queue_url(QueueName = queueName,
                                            QueueOwnerAWSAccountId = queueAccount)['QueueUrl']
        tags = sqs_client.list_queue_tags( QueueUrl = queueURL )
        logger.debug(tags)
        output_bucket = tags['Tags']['target']
    ...
    logger.debug(writer_filename)


    args = ['/opt/bin/pdal', 'pipeline', '--stdin','--debug', '--pipeline-serialization', 'STDOUT', '--writers.%s.filename=/tmp/%s'%(writer_type, writer_filename)]
    ...
    s3 = boto3.resource('s3')
    logger.debug('output bucket: %s' % output_bucket)
...
def pdal_info(event, context):
    ...
    if record['eventSource'] == 'aws:sqs':
        # pipeline comes through record['body'] as text
        pipeline = json.loads(json.loads(record['body']))
        logger.debug('getting pipeline from sqs')
